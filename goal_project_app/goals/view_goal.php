...
$id = $_GET['id'] ?? 0;
$goal = $pdo->prepare("SELECT * FROM goals WHERE id=?");
$goal->execute([$id]);
$goal = $goal->fetch();
...
<div class="container-fluid h-100">
  <div class="row h-100">
    ...
    <nav class="sidebar">
      ...
      <div class="submenu">
        <a href="add_goal.php"><i class="bi bi-plus-circle"></i> Add Goal</a>
        <a href="list_goals.php"><i class="bi bi-list-task"></i> List Goals</a>
      </div>

      <a class="menu-toggle"><i class="bi bi-diagram-3"></i> Projects <i class="bi bi-chevron-down float-end"></i></a>
      <div class="submenu">
        <a href="../projects/add_project.php"><i class="bi bi-plus-circle"></i> Add Project</a>
        <a href="../projects/list_projects.php"><i class="bi bi-list-task"></i> List Projects</a>
      </div>
    ...
    <!-- Main Content -->
    <main class="main-content d-flex flex-column align-items-start">
      <h3 class="mb-4"><i class="bi bi-eye"></i> View Goal</h3>

      <!-- Unified Card: Table + Graph -->
      <div class="card-modern">
        ...
        <table class="table table-hover align-middle mb-0">
          <tbody>
            ...
            <tr>
              <th><i class="bi bi-cash-stack"></i> Target Amount</th>
              <td>$<?= number_format($target, 2) ?></td>
            </tr>
            <tr>
              <th><i class="bi bi-wallet2"></i> Current Amount</th>
              <td>$<?= number_format($current, 2) ?></td>
            </tr>
            <tr>
              <th><i class="bi bi-clipboard-check"></i> Status</th>
              <td><span class="badge bg-success"><?= htmlspecialchars($goal['status']) ?></td>
            </tr>
            <tr>
        ...
        <div class="mt-3 text-start">
          <a href="edit_goal.php?id=<?= $goal['id'] ?>" class="btn btn-warning"><i class="bi bi-pencil"></i> Edit</a>
          <a href="list_goals.php" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Back</a>
        </div>
